...
function SearchResult() {
  ...
  const dispatch = useDispatch();
	...
	useEffect(() => {
		...
		if (spotifyAccessToken && keyword) {
			getUsersSearch(keyword);
		}
	}, [spotifyAccessToken, location.search]);
              ...
              <a className="nav-link player-bg" href="#">
                曲風
					...
					<div className="row mb-5">
						{searchData.artists &&
							searchData.artists?.items.length > 0 && (
								<div className="col-lg-4 col-12">
									...
									<Link
										className="p-5 player-bg rounded-4 mb-5 mb-lg-0 d-block text-decoration-none"
										to={`/player/artist_detail?id=${searchData.artists?.items[0]?.id}&name=${searchData.artists?.items[0]?.name}`}
									>
										<img
						...
						<div className={searchData.artists && searchData.artists?.items.length  > 0 ? "col-lg-8 col-12" : "col-12"}>
					...
					<div className="row mb-5">
						<div className="col-12">
							...
							<div className="row">
								{(() => {
									const items =
										searchData.albums?.items?.filter(
											(album) => album
										) || [];
									const maxItems = showAllAlbums
										? items.length
										: 4;
					...
					{searchData.artists && searchData.artists?.items.length > 0 && (
						<div className="row mb-5">
							...
							{(() => {
								const items =
									searchData.artists?.items?.filter(
										(artist) => artist
									) || [];
								const maxItems = showAllArtist
									? items.length
									: 4;
								const slicedItems = items.slice(0, maxItems);
						...
						<div className="col-12">
							...
							<div className="row">
								{(() => {
									...
									const maxItems = showAllPlaylist
										? items.length
										: 4;
									const slicedItems = items.slice(
										0,
